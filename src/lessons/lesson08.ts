...
/**
 * Lesson 8 — Slice by any dimension (the capstone payoff).
 *
 * Lesson 7 built mart_monthly_sales: the facts sliced by *time*. But the
 ...
 * dims the learner built in L3 were never used in a deliverable; this lesson
 * is where the star earns its keep.
 *
 * It ties the whole course together:
 *   - L4 star    : FK → PK join brings the dim attribute to the metric
 *   - L6 fan-out : that join is grain-safe *because* product_id is a unique PK
 *   - L5 / L3b   : to keep a category that never sold, anchor on the dim (spine)
 *
 ...
 *   total     1060  = the L6 gross_sales — the dim join inflates nothing
 *
 * preMaterialize rebuilds the L3/L4 model layer AND the L7 monthly mart, so
...
This is the return on every decision you've made: declare the grain, classify the columns, push attributes into dims, keep facts lean, join without breaking grain. Do that, and "revenue by category", "revenue by price band", "revenue by state" are all one \`GROUP BY\` away — no new modeling, no duplicated rules, no drift.`,
  ...
  // Rebuild the full model layer the learner assembled across L3/L4, plus the
  // L7 monthly mart, so this lesson can slice the star and the finale can show
...
ORDER BY gross_sales DESC;`,
    ...
    },
    {
      kind: 'checkpoint',
      id: 'dim-join-is-safe',
      question: `You just joined \`fact_order_items\` to \`dim_products\`. In Lesson 6, a JOIN turned 1060 into 1800 (fan-out). Did *this* join inflate \`gross_sales\` the same way?`,
      options: [
...
SELECT * FROM mart_sales_by_category ORDER BY gross_sales DESC;`,
    ...
    },
    {
      ...
      options: [
        ...
        'Only after its first sale — the query is anchored on `fact_order_items`, so a category with no items produces no row. To show it at 0 from day one, anchor on `dim_products` and `LEFT JOIN` the fact (the spine pattern from the calendar dim and from Eve in Lesson 5).',
        'Never — a mart can\'t represent a category with zero sales',
        'Immediately, as long as you wrap the SUM in `COALESCE`',
      ],
      correctIndex: 1,
      explanation: `Because the query starts \`FROM fact_order_items\`, it can only show categories that actually sold — the exact "INNER JOIN loses rows" trap from Lesson 5, one layer up. Flip the anchor: \`FROM dim_products p LEFT JOIN fact_order_items i ...\`, gate the metric so no-match rows contribute 0, and every category in the catalog appears whether or not it sold — the same calendar-spine move you made in the side quest, the same \`LEFT JOIN\` that kept Eve. **The dim is the spine; the fact fills it in.**

And that's the whole lab in one sentence: you modeled grain-honest dims and facts *once*, and now you slice the same numbers by **time** (Lesson 7) or by **any attribute** (here) — adding a new cut never means re-modeling. That is what the star bought you.`,
    },
  ],
}
