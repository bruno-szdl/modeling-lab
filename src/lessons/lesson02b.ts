...
import sketch from '../sketches/lesson02b.svg?raw'

/**
 * Lesson 2b — Side quest: the staging layer.
 *
 * Optional. Sits between L2 (column roles) and L3 (dims), where the
 * raw -> staging -> models path is first relevant. The rest of the lab
 * folds staging into the dim/fact build (clean seeds); this side quest is
 ...
 */
const lesson02b: Lesson = {
  id: 2.5,
  title: 'Side quest: the staging layer',
  schemaSketch: {
    svg: sketch,
...
What staging is **not**: it isn't modeling yet. No joins, no derived business rules, no \`price_band\`, no dims. It just makes the raw table clean and predictable so the dim and fact you build next can assume good inputs. (The rest of this lab folds this cleanup straight into the dim/fact build to keep the focus on modeling decisions — this side quest is the one place you do it by hand.)`,
  ...
  // No seeds: this side quest works against one deliberately messy table so it
