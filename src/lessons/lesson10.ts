...
ORDER BY month;`,
    ...
    },
    {
      kind: 'sql',
      id: 'payments-monthly',
      prompt: `Step 2: aggregate \`fact_payments\` to month grain. Three columns: \`paid_revenue\` (SUM where status='paid'), \`paid_orders\` (COUNT DISTINCT order_id where status='paid' — remember Lesson 9: payments and orders are 1:N), and \`refunded_amount\` (refund amounts expressed as positive numbers).`,
      starterSql: `SELECT
