...
/**
 * Lesson 7 — Joins that don't break grain.
 *
 ...
 * rows (6 -> 8) — opens lesson 8, where that same multiplication lands on
 * a metric and becomes fan-out. Keeping "lose rows" here and "multiply
 * rows" there makes each lesson exactly one idea. (Lesson 6, just before
 * this one, named the key that join safety rests on: a unique PK.)
 *
...
That's the "lose rows" half of join correctness. The opposite failure — a join that silently *multiplies* rows — opens **Lesson 8**, and from there it's a short step to the most expensive bug in analytics.`,
    },
  ],
}
